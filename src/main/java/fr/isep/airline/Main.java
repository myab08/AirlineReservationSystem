package src.main.java.fr.isep.airline;

import src.main.java.fr.isep.airline.models.*;
import src.main.java.fr.isep.airline.services.GestionCompagnie;

import java.util.List;

public class Main {
    public static void main(String[] args) {
        // Initialisation du service de gestion (notre base de données temporaire)
        GestionCompagnie systeme = new GestionCompagnie();

        System.out.println("=== Initialisation du Système de Réservation ===");

        // 1. Création des infrastructures (Aéroports et Avions)
        Aeroport cdg = new Aeroport("Charles de Gaulle", "Paris", "Terminal 2E"); 
        Aeroport jfk = new Aeroport("John F. Kennedy", "New York", "Terminal 4"); 
        Avion boeing = new Avion("F-GSPX", "Boeing 777", 300);

        // 2. Création du personnel (Employés)
        Pilote pilote = new Pilote("EMP001", "Jean Dupont", "Paris", "jean@isep.fr", "P-2024", "2024-01-01", "Licence-ATPL");
        PerCabine cabine = new PerCabine("EMP002", "Sophie Martin", "Lyon", "sophie@isep.fr", "C-2024", "2024-02-15", "Qualification-Sécurité");

        // 3. Planification d'un Vol
        Vol volAF123 = new Vol("AF123", "Paris", "New York", "2026-05-12 10:00");// 
        systeme.planifierNouveauVol(volAF123);
        
        // Affectation de l'avion et de l'équipage au vol
        if (boeing.verifierDisponibilite()) {
            boeing.affecterVol();
            pilote.affecterVol(); 
            cabine.affecterVol(); 
            System.out.println("Équipage et avion affectés au vol " + volAF123.getNumeroVol());
        }

        // 4. Gestion d'un Passager et d'une Réservation
        Passager passagerAlice = new Passager("PASS001", "Alice Bernard", "Issy", "alice@test.com", "123456789");
        systeme.ajouterPersonne(passagerAlice);

        // Alice réserve le vol AF123
        Reservation resaAlice = new Reservation("RES-001", "2026-04-17", passagerAlice, volAF123); 
        passagerAlice.reserverVol(resaAlice);
        volAF123.ajouterPassager(passagerAlice);

        // 5. Affichage des informations (Read)
        System.out.println("\n--- Détails de la réservation ---");
        passagerAlice.obtenirInfos();   
        System.out.println("Statut de la réservation : Confirmée");
        
        System.out.println("\n--- Listing des passagers du vol ---");
        volAF123.listingPassager(); 

        // 6. Bonus : Statistiques
        System.out.println("\n--- Rapport d'activité ---");
        systeme.genererRapport(); 
    }
}